...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Visualize Training Loss for All KMER DNABERT Models"
    )
    parser.add_argument(
        "-i",
        "--input_dir",
...
# Process each k
for k in [3,4,5,6]:
    input_file = BASE_INPUT / f"k{k}" / f"train_metrics.json"
    output_fn = f"k{k}"
    
    module = "scripts.analysis.visualize.training.per_log_step"
    metric_key = "loss"
    
    key_label = "Training Loss"
    graph_title = f"Training Loss Curve for DNABERT (K={k})"
